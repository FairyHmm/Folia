...
import { TIER_PRESETS, CORE_TOKENS, ACTION_SHAPE } from "../utils/graphStyleTokens";
...
export function resolveNode2D(node, scale, display = {}) {
  ...
  const radius = tier.multiplier * styles.sizeMultiplier * (display.nodeSize ?? 1);
  const labelPosition = styles.labelPosition || "below";
  const textYOffset = labelPosition === "inside"
    ? 0
  ...
  s.glowRadius          = 1.5 * radius * (isAction ? 1.5 : tier.glowMultiplier) * (display.glowSize ?? 1);
  s.glowInner           = styles.color;
  s.glowOpacity         = display.glowOpacity ?? CORE_TOKENS.glowOpacity3D;
  s.ringRadius          = styles.ringColor ? radius * styles.ringMultiplier * (display.ringSize ?? 1) : 0;
