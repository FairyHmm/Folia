...
export function parseRoleSchema(roleSchema, targetRoles) {
  ...
  const filteredRoles = roleSchema.filter((r) =>
    targetRoles.includes(r.title)
  );

  filteredRoles.forEach((role) => {
    const { title } = role;
    registerNode(title, title, StructuralTier.ROLE.tier, {
      type: StructuralTier.ROLE.type,
    });

    Object.entries(role.domains).forEach(([domain, modules]) => {
      registerNode(domain, domain, StructuralTier.DOMAIN.tier, {
        type: StructuralTier.DOMAIN.type,
      });
      links.push({ source: title, target: domain });

      Object.entries(modules).forEach(([module, skills]) => {
        registerNode(module, module, StructuralTier.MODULE.tier, {
          type: StructuralTier.MODULE.type,
        });
